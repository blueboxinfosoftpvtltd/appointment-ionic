...
var LoginPage = /** @class */ (function () {
    function LoginPage(toastController, router, loadingController, authservice, storage, menuCtrl) {
    ...
    LoginPage.prototype.Login = function () {
        ...
        if (this.username == '' || this.username == undefined || this.username == null) {
            ...
            this.authservice.doLogin(this.username, this.password).subscribe(function (res) {
                _this.res = res;
                if (_this.res.Code == 200) {
                    _this.storage.set("userid", _this.res.pkemployeeid);
                    _this.storage.set("dealerid", _this.res.DealershipId);
                    _this.dealers = _this.authservice.getdealers();
                    for (var i = 0; i < _this.dealers.length; i++) {
                        if (_this.dealers[i].DealershipId == _this.res.DealershipId) {
                            _this.dealername = _this.dealers[i].DealershipName;
                            console.log(_this.dealername);
                            _this.storage.set("dealername", _this.dealername);
                        }
                    }
                    if (_this.res.DesignationName == "General Manager") {
                    ...
                    _this.router.navigate(['/home'], { queryParams: object });
                }
                else if (_this.res.Code == 400) {
                    _this.authservice.dismissLoading();
                    _this.authservice.showToast(_this.res.msg);
                }
            });
        }
    };
    ...
    LoginPage.prototype.rememberme = function (event) {
        console.log(event);
