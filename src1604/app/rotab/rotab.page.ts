...
ionViewWillLeave(){
  ...
  continue(){
    if(this.min == null || this.min == undefined || this.min == ""){
      ...
      var contactarr =[];
      if(this.cemail == true){
        contactarr.push("Email");
      }
      if(this.cphone == true){
        contactarr.push("Phone");
      }
      if(this.csms == true){
        contactarr.push("SMS");
      }
      var contstr = contactarr.join();
    let data = {
      //'opcode' : finalopcode,
      'dlrid':this.dealerid,
      'cid':this.CustomerId,
    ...
    if(this.opcodes){
      if(this.opcodes.image == "no"){
        this.authservice.createtabro(this.opcodes.cid,this.opcodes.vin,this.min,this.sno,this.tagno,this.pono,contstr,this.cname,this.opcodes.add1,this.opcodes.city,this.opcodes.state,this.opcodes.zip,this.opcodes.homeno,this.opcodes.workno,this.opcodes.email,this.opcodes.year,this.opcodes.make,this.opcodes.model,this.opcodes.license,this.opcodes.color,this.opcodes.uid,this.mout,this.wsdate,this.wedate,this.opcodes.dlrid,this.rnotes,this.addrolist,this.opcodes.carimage,this.selectval,this.rono,this.pdate).subscribe(res =>{
          console.log(res);
          this.authservice.dismissLoading();
          this.rores = res;
  ...
  updatero(){
   ...
   // }
    ...
    this.authservice.presentLoading();
    this.authservice.updatetech(this.sno,this.rono,this.rline,this.opdesc,this.tagno,this.cc,this.rcode,this.labourname,this.techno,this.shours,nline,this.dealerid).subscribe(res =>{
      var rres;
      rres = res;
      this.addrolist =rres;
      this.authservice.dismissLoading();
