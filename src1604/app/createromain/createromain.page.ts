...
@Component({
  selector: 'app-createromain',
  templateUrl: './createromain.page.html',
  styleUrls: ['./createromain.page.scss'],
})
export class CreateromainPage implements OnInit {
...
SelectOP(event,data,i){
  ...
  //     this.checkop.push(data);
  //   }
  //   else{
  //     for(let i = 0;i <this.checkop.length;i++){
  //       if(this.checkop[i] == data){
...
continuero(){
  ...
  var finalopcode = [];
            for(let i = 0 ; i<this.newopcodelist.length;i++){
              if(this.newopcodelist[i].isChecked == true){
                finalopcode.push(this.newopcodelist[i]);
              }
            }
            if(finalopcode.length == 0){
              this.authservice.showToast("Please select opcode");
