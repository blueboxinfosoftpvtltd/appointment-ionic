...
export class CreateappointmentPage implements OnInit {
  ...
  @ViewChild("buttonContainerRef", { static: false })
  buttonContainerRef: ElementRef;
  ...
  ngOnInit() {
    ...
    this.authservice.getcustidvin().subscribe((data) => {
      ...
      if (this.rescustvin.data) {
        ...
        if (
          this.rescustvin.data.Notes != null ||
          this.rescustvin.data.Notes != "" ||
          this.rescustvin.data.Notes != undefined
        ) {
          this.sub = this.rescustvin.data.Notes;
        }
        if (
          this.rescustvin.data.NotesList != null ||
          this.rescustvin.data.NotesList != "" ||
          this.rescustvin.data.NotesList != undefined
        ) {
          var note = [];
          var index;
  ...
  ChangeWheel(event) {
    ...
    if (this.wheel != null && this.selectedtire == undefined) {
      ...
      // show alert if all values selected
      if (this.lfw && this.rfw && this.lrw && this.rrw
        && this.lft && this.rft && this.lrt && this.rrt) {
        this.alertController.create({
          header: "Warning",
          message: 'You can only update in edit mode.',
          buttons: ["Ok"]
        }).then(alert => {
          alert.present();
        });
  ...
  edittw(val) {
    console.log(this.rescustvin.data);
  ...
  scrollToCenter(_page: any) {
    ...
    console.log("scroll : " + scrollWidth + " client : " + clientWidth);
